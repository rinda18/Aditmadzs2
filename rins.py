...
aditmadzs = LineClient(authToken='EwjA7BGA87lPrc4L0zU9.grzYgOnTdKvSkrQ7FTsKMq.TmsMcUZV6F1pLmV4TxQ5RDSN8NwxUg/x9iFFmCoYF6s=')
...
#ks = LineClient()
ks = LineClient(authToken='EwMOswNrySknlz9DLjP3.zp81iCnJRS6DWRFwE7rbCW.pjYodEkBT4a54+NUparCYdfQvGrY0Pgi/9JYhFwtU4E=')
ks.log("Auth Token : " + str(ks.authToken))
channel1 = LineChannel(ks)
ks.log("Channel Access Token : " + str(channel1.channelAccessToken))

#kr = LineClient()
kr = LineClient(authToken='EwLmlQvl5L2onz67a9be.xmuvUN9DywEZbNRmpsxChG.AQNksnA/k4V9IoaqgsLYj6Kb8bRi65hVAMXtAKuY82U=')
kr.log("Auth Token : " + str(kr.authToken))
channel1 = LineChannel(kr)
kr.log("Channel Access Token : " + str(channel1.channelAccessToken))
                    ...
                    if text is None:
                        ...
                        if cmd == "helpz":
                          if wait["selfbot"] == True:
                               ...
                               #creator = aditmadzs.getContact(poey)
                               #sendMentions(to, str(helpMessage), [poey])
                               aditmadzs.sendMessage(msg.to, str(helpMessagez))
...
# Secure 1 : Anti Inviting, Secure 2 : Anti Kicking, Secure 3 : Anti Cancelling, Secure 4 : Anti Joined, Secure 5 : Anti Qr Update
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                kr.sendMessage(msg.to,"こんにPUYちは。")
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                G.preventedJoinByTicket = True
                                aditmadzs.updateGroup(G)
                                kr.sendMessage(to, "すべてのこんにちは、私はPUY")

                        elif cmd == "@bye":
                          if wait["selfbot"] == True:
                            #if msg._from in admin:
                                G = aditmadzs.getGroup(msg.to)
                                kr.sendMessage(msg.to, "もう一度会いましょう\n"+str(G.name))
                          ...
                          if wait["selfbot"] == True:
                                ...
                                kr.sendMessage(msg.to, "Dadah "+str(G.name))
                                #aditmadzs.leaveGroup(msg.to)
                                kr.leaveGroup(msg.to)
                          ...
                          if wait["selfbot"] == True:
                                 ...
                                 aditmadzs.sendMessage(msg.to, "Checkread Is now On.")

                        elif cmd == "checkread off":
                          if wait["selfbot"] == True:
                            #if msg._from in admin:
                                 tz = pytz.timezone("Asia/Jakarta")
                                 timeNow = datetime.now(tz=tz)
                                 ...
                                 aditmadzs.sendMessage(msg.to, "Checkread Off.")
                          ...
                          if wait["selfbot"] == True:
                              if settings["autoJoinTicket"] == True:
                                 ...
                                 for ticket_id in n_links:
                                     ...
                                     group1 = ks.findGroupByTicket(ticket_id)
                                     ki.acceptGroupInvitationByTicket(group1.id,ticket_id)
                                     kk.acceptGroupInvitationByTicket(group2.id,ticket_id)
                                     kc.acceptGroupInvitationByTicket(group3.id,ticket_id)
                                     km.acceptGroupInvitationByTicket(group4.id,ticket_id)
                                     ks.acceptGroupInvitationByTicket(group5.id,ticket_id)
